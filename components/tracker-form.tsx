...
import { Car, Utensils, Zap, Plus, Leaf, TrainFront, X, Trash2, Lightbulb, Sparkles, TrendingDown, Droplets, TreePine, Recycle, Target, Heart, Wind, Sun, Bike, Flame } from 'lucide-react';
...
export const tipIconMap: Record<Tip['icon'], React.ElementType> = {
  leaf: Leaf,
...
export const tipTypeStyles: Record<Tip['type'], { bg: string; border: string; iconBg: string; iconColor: string; titleColor: string; textColor: string }> = {
  positive: { 
  ...
  return (
    <>
      ...
      <div className="mb-6 space-y-3">
        <h2 className="text-xs font-bold text-slate-600 dark:text-slate-500 uppercase tracking-wider">1-Tap Quick Actions</h2>
          ...
          <div className="space-y-4 flex-1">
            ...
            <div>
              ...
              <input 
                ...
                placeholder="e.g. 15" 
                className="w-full p-2 border border-slate-200 dark:border-slate-800 rounded text-sm text-slate-700 dark:text-slate-200 bg-slate-50 dark:bg-slate-800/40 focus:outline-none focus-visible:border-emerald-500 focus-visible:ring-2 focus-visible:ring-emerald-500 transition-colors" 
              />
            </div>
          </div>
          <button 
              ...
              {visibleLogs.map((log) => (
                <div key={log.id} className="flex justify-between items-center text-sm p-3 bg-emerald-50/80 dark:bg-emerald-950/20 text-emerald-800 dark:text-emerald-300 rounded border border-emerald-100/70 dark:border-emerald-900/30 group">
                  <div className="flex flex-col gap-1">
                    ...
                    <span className="text-xs font-bold text-emerald-700 dark:text-emerald-400 bg-emerald-100 dark:bg-emerald-950/40 px-2 py-0.5 rounded w-fit">Success</span>
                  </div>
                  <div className="flex items-center gap-2">
                    <button
                      onClick={() => deleteLog(log.id)}
                      className="p-1.5 text-emerald-700 dark:text-emerald-400 hover:text-red-600 dark:hover:text-red-400 hover:bg-white dark:hover:bg-slate-800 rounded transition-colors"
